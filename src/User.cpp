...
std::string User::input(const std::string &main_prompt, const std::string &data_type, const std::string &other_option) {
    while (true) {
        if (data_type == "std::string")
            ...
            printf("Enter (1) for %s, (2) for %s or (0) to exit:", main_prompt.c_str(), other_option.c_str());
        std::string rawInput;
        ...
        if (data_type == "bool" && rawInput != "1" && rawInput != "2" && rawInput != "0") {
            printf("Can only accept 1, 2, or 0\n");
            continue;
        }
        return rawInput;
    }
...
bool User::change_pin(const bool &first_time) {
    constexpr int MAX_PASS_CHANGE_TRIALS = 3;
    int trials = 0;
    while (++trials <= MAX_PASS_CHANGE_TRIALS) {
        if (!first_time) {
